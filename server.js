...
app.post('/generate-pair-code', async (req, res) => {
  ...
  if (!phoneNumber) return res.json({ error: 'Phone number required' });
  ...
  try {
    ...
    console.error(err);
    res.json({ error: 'Failed to send WhatsApp message. Make sure the bot is online.' });
  }
});
...
app.post('/verify-pair-code', async (req, res) => {
  ...
  if (!phoneNumber || !pairCode) return res.json({ error: 'Missing fields' });
  if (pairCodes[phoneNumber] !== pairCode) return res.json({ error: 'Invalid pair code' });
...
client.on('ready', () => console.log('✅ Jinx4G WhatsApp Bot Ready'));
client.initialize();
